...
defineSupportCode(function ({ Given, When, Then }) {
    ...
    Given(/^I have a student "([^\"]*)" with CPF "(\d*)" in the students list$/, async (name, cpf) => {
    ...
    Then(/^I cannot see "([^\"]*)" with CPF "(\d*)" in the students list$/, async (name, cpf) => {
        ...
        await allalunos.filter(elem => pAND(sameCPF(elem,cpf),sameName(elem,name))).then(elems => expect(Promise.resolve(elems.length)).to.eventually.equal(0));
    });

    When(/^I try to remove the student "([^\"]*)" with CPF "(\d*)"$/, async (name, cpf: string) => {
        await element(by.id(cpf)).click();
    });

    Then(/^I can see "([^\"]*)" with CPF "(\d*)" in the students list$/, async (name, cpf) => {
        var allalunos : ElementArrayFinder = element.all(by.name('alunolist'));
        await allalunos.filter(elem => pAND(sameCPF(elem,cpf),sameName(elem,name))).then(elems => expect(Promise.resolve(elems.length)).to.eventually.equal(1));
    });
})
